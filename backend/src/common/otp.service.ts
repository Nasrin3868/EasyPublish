...
export async function sendOtp(email: string): Promise<OtpResult> {
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        user: 'nasrinkichlu3868@gmail.com', // Replace with your email
        pass: 'vyhf yfwi zpqe rrhx', // Replace with your email password or app password
      },
    });

    // Email content
    const mailOptions = {
      from: '"Easy Publish" nasrinkichlu3868@gmail.com', // sender address
    ...
    return { success: true, message: `OTP sent successfully to ${email}`,otp:otp };
  } catch (error) {
